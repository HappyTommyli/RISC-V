...
main_loop:
    lw s4, 0(s0)         # 【修改】使用 s4 儲存按鍵狀態，避免被 is_ladder 覆蓋

check_up:
    andi t1, s4, 1       # 【修改】t0 改為 s4
...
do_frame:
    ...
    # 【修改】大幅增加延遲，讓畫面刷新率降到合理範圍
    addi t0, zero, 30
delay_outer:
    lui t1, 4            # lui 4 代表 4 * 4096 = 16384 次內部迴圈
delay_inner:
    addi t1, t1, -1
...
frame_render:
    ...
    addi t0, zero, 0
    addi t4, zero, 1024  # 【修改】將 1024 移出迴圈外，提升效能
copy_map_loop:
    add t1, s3, t0
    lbu t2, 0(t1)
    add t3, s2, t0
