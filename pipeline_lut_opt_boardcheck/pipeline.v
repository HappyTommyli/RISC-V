...
module pipeline (
    ...
    // Fixed check target for board verification
    localparam CHECK_ADDR  = 32'h000012FC;
    localparam CHECK_VALUE = 32'h00004560;
    // IF/ID outputs
    wire [31:0] if_id_pc;
...
//
    ...
    // Pass latch: when a store writes expected value to CHECK_ADDR, keep LED on.
    always @(posedge clk or posedge rst) begin
        if (rst) begin
            pass_led <= 1'b0;
        end else if (ex_mem_write_reg && (ex_mem_alu_result_reg == CHECK_ADDR) &&
                     (mem_rs2_data == CHECK_VALUE)) begin
            pass_led <= 1'b1;
        end
    end
