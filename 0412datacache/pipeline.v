...
//
    ...
    assign wb_valid = (mem_wb_instr_reg != 32'h00000000);
//
    assign miss = (ex_branch_taken != ex_predicted_take);
    wire id_predict_take_final =
    id_jump ? 1'b1 :
    (id_branch ? dyn_pred_taken : 1'b0);
//
wire        cache_stall;
wire        pipeline_stall;
assign pipeline_stall = hazard_stall | cache_stall;
    ...
    IF u_IF (
        ...
        .predicted_take(id_predict_take_final),
    ...
    always @(posedge clk or posedge rst) begin
        if (rst || miss) begin
            ...
            ex_predicted_take <= id_predict_take_final;
        end
    end

// Decode whether ID/EX 指令是 branch
wire id_is_branch = id_branch;  // CU 已經解出 branch [file:20]
wire ex_is_branch = ex_branch;  // ID/EX 控制暫存器裡的 ex_branch [file:64]

// EX stage 看到的 PC 就是 id_ex_pc [file:64]
branch_predictor #(
    .INDEX_BITS(6)       // 64-entry BHT，之後可以調
) u_bpred (
    .clk          (clk),
    .rst          (rst),

    // Query: ID stage
    .id_pc        (if_id_pc),     // branch 在 ID 階段的 PC
    .id_is_branch (id_is_branch),
    .id_pred_taken(dyn_pred_taken),

    // Update: EX stage
    .ex_pc        (id_ex_pc),     // 同一條 branch 進 EX 時的 PC
    .ex_is_branch (ex_is_branch & ~miss), // 只有不是被 flush 掉的 branch 才更新
    ...
    always @(posedge clk or posedge rst) begin
        if (rst) begin
            ...
            mem_wb_instr_reg     <= mem_wb_instr_reg;
        end else begin
